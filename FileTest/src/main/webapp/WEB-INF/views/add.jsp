...
<style>
	...
	#attach-zone{
		boarder: 1px solid var(--border-color);
...
<body>
	...
	function checkFile(filename, filesize) {
		...
		const regex = /(.*?)\.(exe|sh)$/gi; //test.exe, test.sh > 이런 확장자 명은 들어가지 못한다.
		
		if(filesize > maxsize){
			alert('해당 파일의 크기가 10MB 이하만 가능 합니다.');
			return true;
		}
	...
	$('#form1').submit(() => {
		...
		//alert($('input[name=attach]')[0].file[0].name);	
