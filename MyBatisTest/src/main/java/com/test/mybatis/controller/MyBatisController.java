...
public class MyBatisController {
	...
	@GetMapping("/m14.do")
	/*public String m14(Model model, @RequestParam(name = "type", defaultValue = "1") int type) {*/
	public String m14(Model model, Integer type) {
		...
		/*List<MyBatisDTO> list = new ArrayList<MyBatisDTO>(); //dao.m14(type);*/
	...
	public String m15(Model model, String column, String word) {
		...
		//검색
		// - m15.do?column=컬럼명&woed=검색어
		
		// - m15.do?column=name&word=강아지		> 동등비교
		// - m15.do?column=gender&word=m		> 동등비교
		// - m15.do?column=seq&word=3			> 동등비교
		
		// - m15.do?column=seq&word=2			> 우위비교
		
		// - m15.do?column=address&word=강동		> 패턴비교
...
//		dao.addDetail(ddto);
	...
	public String m20(Model model) {
		...
		//방법 1. (dto가 너무 많아진다. 사용 x)
		// AddressDTO + InfoDTO = 새로운 DTO만들기
